...
> **Note:** release tags are not yet GPG/SSH-signed — that's tracked as a pending improvement in `ci.yml`, not yet implemented. Property 2 above currently means "matches a `vX.Y.Z` tag," not "matches a signed one." This note will be removed once signing is wired up.
